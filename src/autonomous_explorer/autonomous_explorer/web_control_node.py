        ...
        try:
            ...
            if command == 'start_mapping':
                ...
                if os.path.exists(map_path):
                    ...
                    # Send robot to explore - send multiple waypoints
                    # For now, send a simple exploration pattern
                    if self.send_navigation_goal(2.0, 2.0):
                        await websocket.send(json.dumps({
                            'status': 'exploration_started', 
                            'area': area_name,
                            'message': 'Robot moving to exploration waypoint'
                        }))
                        self.get_logger().info(f'✓ Robot navigating in {area_name}')
                    else:
                        await websocket.send(json.dumps({
